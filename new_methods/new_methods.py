...
class My_NuScenes(NuScenes):
    ...
    def render_pointcloud_in_scene(self, sample_token: str, 
        ...
        self.explorer.render_pointcloud_in_scene(sample_token, dot_size, nsweeps=nsweeps,
    ...
    def check_depth(self,
                    ...
                    lidarseg_preds_bin_path: str = None,
                    show_panoptic: bool = False):
        return self.explorer.check_depth(
                    sample_token,
                    boundary,
                    dot_size,
                    pointsensor_channel = pointsensor_channel,
                    ...
                    show_panoptic = show_panoptic)
    def abc(self, n):
        return self.explorer.abc(n)
    
    def make_points_list(self,
        sensor: str = 'r',
        frame_idx: int = 0,
        nsweeps: int = 1):
        return self.explorer.make_points_list(
            sensor = sensor,
            frame_idx = frame_idx,
            nsweeps = nsweeps)
...
class My_NuScenesExplorer(NuScenesExplorer):
            ...
            if len(points_result_R[channel][0]) > 0:
                ...
                for i in range(len(d_R)):
                    ...
                    if True in mask:
                        ...
                        depth_diff = abs(Rapo[2][i] - Lipo[2][index])
    ...
    def map_multiview_pointcloud_to_image(self,
        ...
        for j in range(len(cam_list)):
            ...
            for k in range(len(pointsensor_token)):
                ...
                poserecord = self.nusc.get('ego_pose', ref_sd_record['ego_pose_token'])
    ...
    def make_points_list(self,
            ...
            frame_idx: int,
            nsweeps: int) -> list:
        # make every points included in one frame to 'LiDAR coordinate'(result of multisweep) -> 'ego coordinate'
        ...
        # lidar infos
        # lidarsensor = self.nusc.get('sample_data', list(lidar_data.values())[0])
        ...
        for k in range(len(pointsensor_token)):
            ...
            if 'LIDAR_TOP' in pointsensor_channel:
                ...
                pc, times = RadarPointCloud.from_file_multisweep(self.nusc, record, pointsensor['channel'], ref_chan, nsweeps=nsweeps)
            # Points live in the point sensor frame. So they need to be transformed via global to the image plane.
            # First step: transform the pointcloud to the ego vehicle frame for the timestamp of the sweep.
            ...
            # poserecord = self.nusc.get('ego_pose', lidarsensor['ego_pose_token'])
            # pc.translate(-np.array(poserecord['translation']))
            # pc.rotate(Quaternion(poserecord['rotation']).rotation_matrix.T)
            ...
            if len(pc.points[0]) != 0:
                points = pc.points[:3, :]
                points_list = np.hstack((points_list, points))
        
        points_list = points_list.T
    ...
    def render_sample_data_new(self,
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            # Draw circle
            for r in range(10, 151, 10):  # 10m부터 시작하여 150m까지 10m 간격
                if r == 50:
                    color_circle = 'red'
    ...
    def render_sample_new(self,
        ...
        check_path = '/home/byounghun/workspace'
